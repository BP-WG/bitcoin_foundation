...
//!
//! While ECDSA keys are expected to be replaced with Taproot or other future
//! upgrades they are still widely used at the time of writing this library.
//! This module contains the types and methods for handling them correctly and
//! easily.
//!
//! Note that if you're writing a modern Bitcoin application from scratch it may
//! be better to use P2TR - see the [`schnorr`](crate::schnorr) module.
//!
...
//! [`Legacy`] may be (de)serialized as uncompressed and dynamically remembers
//! the format. [`Compressed`] is statically known to be compressed and can not
//! be serialized as uncompressed. Aside from saving a tiny bit of memory, it
//! can statically prevent problems like panics when constructing SegWit v0
//! addresses.
//!
...
impl KeyFormat {
    ...
    #[inline]
    pub fn is_compressed(self) -> bool { self == KeyFormat::Compressed }
...
mod sealed {
    ...
    impl PublicKey for secp256k1::PublicKey {
        #[inline]
        fn public_key(self) -> secp256k1::PublicKey { self }
    }

    impl PublicKey for secp256k1::KeyPair {
        #[inline]
        fn public_key(self) -> secp256k1::PublicKey { self.into() }
    }
    ...
    pub trait PrivateKey: Key {
        ...
        #[inline]
        fn compute_public_key<C: secp256k1::Signing>(
            self,
            context: &Secp256k1<C>,
        ) -> secp256k1::PublicKey {
    ...
    impl PrivateKey for secp256k1::SecretKey {
        #[inline]
        fn private_key(self) -> secp256k1::SecretKey { self }
    }

    impl PrivateKey for secp256k1::KeyPair {
        #[inline]
        fn private_key(self) -> secp256k1::SecretKey { self.into() }

        /// Optimized override skips computing
        #[inline]
        fn compute_public_key<C: secp256k1::Signing>(
            self,
            _context: &Secp256k1<C>,
        ) -> secp256k1::PublicKey {
            self.into()
        }
    }
...
///
/// Old Bitcoin addresses may have internally used an uncompressed public key.
/// This is discouraged in the new software since it wastes money, among other
/// things, but it may be required to recover old coins.
#[derive(Copy, Clone, Debug, Eq, PartialEq, Hash)]
...
impl<K: Key> Legacy<K> {
    /// Constructs the legacy key from the underlying secp256k1 key and format
    /// information.
    ///
    /// **Warning:** make sure to supply the correct key format. Incorrect
    /// format may lead to a different address making spending difficult or
    /// even impossible for non-technical people.
    #[inline]
    pub fn from_raw(key: K, format: KeyFormat) -> Self { Legacy { key, format } }
    ...
    /// As a consequence, improper use can make it harder to spend from the
    /// address, even impossible for non-technical people.
    ///
    /// The method should only be used when this behavior is known to be
    /// correct, e.g. in recovery tools.
    #[inline]
    pub fn force_set_format(&mut self, format: KeyFormat) { self.format = format; }
    ...
    /// correct, e.g. in recovery tools.
    #[inline]
    pub fn force_to_compressed(self) -> Compressed<K> { Compressed::from_raw(self.key) }

    /// Returns true if the keys are equal *regardless of the format*.
    ///
    /// The `Eq` trait takes serialization format into account thus same keys
    /// with different formats are considered **not** equal. This method
    /// ignores the format when comparing.
    #[inline]
...
/// key.
///
/// This key may be used in either P2SH or SegWit v0 addresses which are still
/// widely used but are being replaced by P2TR addresses. New software is
/// encouraged to use P2TR implemented ing the [`schnorr`](crate::schnorr)
/// module but this may still be required to recover old coins.
...
impl<K: Key> From<Compressed<K>> for Legacy<K> {
    fn from(value: Compressed<K>) -> Self { Self::from_raw(value.raw_key(), KeyFormat::Compressed) }
}
...
/// Returned when attempting to convert legacy key into compressed and the
/// legacy key is in uncompressed format.
#[derive(Debug, Clone)]
#[non_exhaustive]
